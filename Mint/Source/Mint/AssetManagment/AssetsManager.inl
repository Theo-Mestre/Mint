...
{
	m_assets.reserve((size_t)Mint::Asset::Type::Count);
}
inline AssetsManager::~AssetsManager()
{
	UnloadAllAssets();
...
{
	static_assert(std::is_base_of<Mint::Asset, T>::value, "T must be a subclass of Asset");
...
inline Ref<T> AssetsManager::GetAsset(const char* _assetName)
{
	...
	auto& assets = m_assets[std::type_index(typeid(T))];
	auto compare = [&_assetName](const Ref<Mint::Asset>& _asset) { return _asset->GetName().c_str() == _assetName; };
...
inline Ref<T> AssetsManager::AddAssets()
{
	...
	{
		m_assets[typeIndex] = std::vector<Ref<Mint::Asset>>();
	}

	std::vector<Ref<Mint::Asset>>& assets = m_assets[typeIndex];
